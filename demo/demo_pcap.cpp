...
void exceptionCallback(const Error& code)
{
  ...
  if (code.error_code == ERRCODE_PCAPEXIT) {
...
int main(int argc, char* argv[])
{
  ...
  param.input_param.pcap_path = "/opt/data/A0-test.pcap";  // Set the pcap file directory
  param.input_param.pcap_repeat = false;                   // Set to no repeat
  ...
  selection.min_distance = 0.0f;
  selection.max_distance = 200.0f;
  driver.setSelectionParam(selection);
  // driver.setDefaultSelectionParam();

  driver.start();  // The driver thread will start

  AG_DEBUG << "Asensing Lidar-Driver Linux pcap demo start......" << AG_REND;
