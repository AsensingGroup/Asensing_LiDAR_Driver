...
void printHelpMenu()
{
  ...
  AG_MSG << "" << AG_REND;
  AG_MSG << "Example: simple_csvsaver -type A0 -pcap /opt/data/A0-test.pcap" << AG_REND;
}
...
void saveCsv(const std::string &csv_path, const PointCloudMsg &cloud)
{
  AG_MSG << "Save point cloud as " << csv_path << AG_REND;

  std::ofstream os(csv_path, std::ios::out | std::ios::trunc);
  os << "Point ID,X,Y,Z,Intensity" << std::endl;
...
void processCloud(void)
{
  while (!to_exit_process)
  {
    ...
    char csv_path[128];
    sprintf (csv_path, "%d.csv", msg->seq);
    saveCsv(csv_path, *msg);
...
{
  AG_TITLE << "------------------------------------------------------" << AG_REND;
  AG_TITLE << "            AG_Driver CSV Saver Version: v" << getDriverVersion() << AG_REND;
  AG_TITLE << "------------------------------------------------------" << AG_REND;
  ...
  AG_INFO << "Asensing Lidar-Driver CSV Saver start......" << AG_REND;
